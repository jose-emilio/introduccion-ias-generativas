# Curso: Introducción a las IAs Generativas

**Duración:** 12 horas (2 jornadas intensivas de 6 horas)
**Modalidad:** Presencial / Virtual sincrónico
**Público:** Profesionales, managers y personas no técnicas

---

## Módulos de Teoría

| Módulo | Secciones |
|--------|-----------|
| [Módulo 1: Fundamentos de IA](modulos/modulo-1-fundamentos-ia/index.md) | 1.1 Qué es la IA · 1.2 Historia y evolución · 1.3 ML, Deep Learning y NLP · 1.4 Automatización inteligente |
| [Módulo 2: Introducción a la IAG](modulos/modulo-2-iag-introduccion/index.md) | 2.1 LLMs, tokens y contexto · 2.2 Alucinaciones y sesgos · 2.3 Modelos actuales · 2.4 RAG y Fine-tuning |
| [Módulo 3: Herramientas y Aplicaciones](modulos/modulo-3-aplicaciones/index.md) | 3.1 Ecosistema de herramientas · 3.2 Prompt engineering · 3.3 Desarrollo asistido · 3.4 Agentes y automatización |
| [Módulo 4: Seguridad, Ética y Gobernanza](modulos/modulo-4-herramientas-seguridad/index.md) | 4.1 Ética y sesgos · 4.2 Regulación y marco legal · 4.3 Gobernanza y seguridad · 4.4 Integración segura |
| [Módulo 5: Conclusiones y Plan de Acción](modulos/modulo-5-conclusiones/index.md) | 5.1 Recapitulación · 5.2 Superando la inercia · 5.3 Plan de acción individual |

## Laboratorios Prácticos

| # | Laboratorio | Módulo relacionado |
|---|-------------|-------------------|
| 1 | [Identificando IA en el mundo real](laboratorios/lab-1-identificando-ia.md) | M1 - Fundamentos |
| 2 | [Alucinaciones, sesgos y deepfakes](laboratorios/lab-2-alucinaciones-sesgos.md) | M2 - IAG |
| 3 | [Creación visual con IAG](laboratorios/lab-3-creacion-visual.md) | M3 - Aplicaciones |
| 4 | [Creación sonora con IAG](laboratorios/lab-4-creacion-sonora.md) | M3 - Aplicaciones |
| 5 | [Productividad con LLMs](laboratorios/lab-5-productividad-llms.md) | M3 - Aplicaciones |
| 6 | [RAG: conocimiento interno con LLMs](laboratorios/lab-6-rag.md) | M3 - Aplicaciones |
| 7 | [Comparativa y evaluación de LLMs](laboratorios/lab-7-comparativa-llms.md) | M4 - Herramientas |
| 8 | [Prompt engineering práctico](laboratorios/lab-8-prompt-engineering.md) | M4 - Herramientas |
| 9 | [Auditoría de riesgos en IAG](laboratorios/lab-9-auditoria-riesgos.md) | M4 - Seguridad |

## Objetivos de aprendizaje

Al finalizar el curso, los participantes serán capaces de:

1. **Explicar** qué es la IA generativa y cómo se diferencia de otros tipos de IA.
2. **Identificar** los principales modelos (LLMs, difusión) y sus casos de uso.
3. **Aplicar** técnicas de Prompt Engineering para obtener mejores resultados.
4. **Evaluar** los riesgos éticos, de sesgo y de seguridad asociados a la IAG.
5. **Diseñar** políticas básicas de uso responsable de IAG en su organización.
6. **Distinguir** entre herramientas públicas inseguras y APIs empresariales protegidas.

## Mapa de Contenidos vs Tiempo

| Bloque | Contenido | Duración |
|--------|-----------|----------|
| 1 | Módulo 1 — Fundamentos de IA + Lab 1 | 1h 15min |
| 2 | Módulo 2 — IA Generativa y LLMs + Lab 2 | 1h 15min |
| 3 | Módulo 3 — Herramientas y Aplicaciones + Labs 3-6 | 1h 15min |
| 4 | Módulo 4 — Seguridad, Ética y Gobernanza + Labs 7-9 | 1h 15min |
| 5 | Módulo 5 — Conclusiones y Plan de Acción | 15min |

## Navegación rápida

- [Inicio](README.md)
- [Módulo 1: Fundamentos de IA](modulos/modulo-1-fundamentos-ia/index.md)
- [Módulo 2: Introducción a la IAG](modulos/modulo-2-iag-introduccion/index.md)
- [Módulo 3: Herramientas y Aplicaciones](modulos/modulo-3-aplicaciones/index.md)
- [Módulo 4: Seguridad, Ética y Gobernanza](modulos/modulo-4-herramientas-seguridad/index.md)
- [Módulo 5: Conclusiones y Plan de Acción](modulos/modulo-5-conclusiones/index.md)
- [Laboratorios](laboratorios/lab-1-identificando-ia.md)
